...
#2024_v4 Isabel
# ‘Base [kWh]’ and ‘Case1 [kWh]’ are duplicates of ‘Opt_DA [kWh]’.

# 'event hour_Base' and  'event hour_Case1' are filled with zeros.

# Some columns are not needed in the baseline file pre-RT optimization. However the column format must match that of the 2022 baseline file since the RT optimization script is written to match that format. The following names and order of the columns is: 
# 'Interval start', 'V0G [kWh]', 'V1G_real [kWh]', 'Opt_DA [kWh]',
#       	'Opt_RT_t0 [kWh]', 'Base [kWh]', 'Case1 [kWh]', 'baseline_Base',
#      	 'baseline_Case1', 'NCD_Base', 'PD_Base', 'NCD_Case1', 'PD_Case1',
#      	 'LMP_DA', 'LMP_RT', 'event hour_Base', 'event hour_Case1'
# So empty columns are created for the remainder of these columns not mentioned above.
