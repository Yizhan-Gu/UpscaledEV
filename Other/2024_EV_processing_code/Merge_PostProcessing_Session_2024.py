...
filename_Input = dir_Input + 'UCSD_AllSites_Merge_PostProcessedInterval_20210504_20240930.csv'
...
for ind_event in range(len(Events)):
    ...
    EnergyDemand_Session_list = EnergyDemand_Session_list + [EnergyDemand_Session_ThisEvent]
    ...
    SessionStart_ThisEvent = data_Merge_ThisEvent['Session start'].iloc[0]
    SessionStart_List = SessionStart_List + [SessionStart_ThisEvent]
    

# =============================================================================
# kWh = 3 #4,433 for 0.1 kWh, 2,235 for 1 kWh, 943 for 3 kWh
# AbnormalEvent_kWh = pd.Series(EnergyDemand_Difference_list)[pd.Series(EnergyDemand_Difference_list)>kWh]
# AbnormalEvent_SessionStart = pd.Series(SessionStart_List)[pd.Series(EnergyDemand_Difference_list)>kWh]
# 
# # Plot histogram of EnergyDemand_Difference_list
# ax1 = plt.hist(AbnormalEvent_kWh) #, bins = list(np.linspace(-15, 5, 21)), density=True)
...
# ax2 = plt.hist(pd.to_datetime(AbnormalEvent_SessionStart)) #, bins = list(np.linspace(0, 70, 71)))
# 
...
# =============================================================================
    ...
    #if EnergyDemand_Difference_ThisEvent > 0:
    #if EnergyDemand_Difference_ThisEvent > 3:
        EnergyDemand_Difference_list
...
ax1 = plt.hist(EnergyDemand_Difference_list, bins = list(np.linspace(-15, 5, 21)), density=True)
#plt.title('Discrepency of Session Energy Demand', fontsize = 12)
plt.xlabel('Discrepancy of Energy Demand [kWh]')
...
# input: Merge_PostProcessed; output: Merge_PostProcessedSession
#################################################################################
#################################################################################
...
filepath = Path(dir_Output + 'UCSD_AllSites_Merge_PostProcessedSession_20210504_20240930.csv')  
...
# Notes:
    # Events_data_Sessions = 97,056
    # Events (Merge) = 69,844
#################################################################################


dir_Input2 = os.path.join("C:/Users/Anne/Desktop/Total/Data/EV_PF_UCSD/2024/")
filename_Input2 = dir_Input2 + 'UCSD_AllSites_Sessions_wCarInfo_20210501_20240930.csv'
data_Sessions= pd.read_csv(filename_Input2)



filename_Input2 = dir_Input2 + 'UCSD_AllSites_Merge_PostProcessedSession_20210504_20240930.csv'
data_Merge_2= pd.read_csv(filename_Input2)



Events_data_Sessions = data_Sessions["10-digit UID"].unique() #97,056
Events_data_Merge2 = data_Merge_2["10-digit UID"].unique()    #69,844

data_Sessions2 = data_Sessions[data_Sessions["10-digit UID"].isin(Events_data_Merge2)]
Events_data_Sessions2 = data_Sessions2["10-digit UID"].unique() #69,844

for ind_event in range(len(Events_data_Merge2)):
    #ind_event = 0 
    EnergyDemand_ThisSession =  data_Merge_2["kWh delivered"][data_Merge_2["10-digit UID"] == Events_data_Merge2[ind_event]]
    data_Sessions2["kWh delivered"][data_Sessions2["10-digit UID"] == Events_data_Merge2[ind_event]]  = EnergyDemand_ThisSession.iloc[0]
...
dir_Output2 = os.path.join("C:/Users/Anne/Desktop/Total/Data/EV_PF_UCSD/2024/")
filepath2 = Path(dir_Output2 + 'UCSD_AllSites_Sessions2_wCarInfo_20210504_20240930.csv')         #20928.csv')  
