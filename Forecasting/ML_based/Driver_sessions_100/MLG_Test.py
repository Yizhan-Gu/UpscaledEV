...
for i in range(len(User_Data)):
    ...
    for j in range(10,len(Data_User)):
        
        train_sessions_data = Data_User.head(j)
        train_sessions_data = train_sessions_data[['Arrival Hour', 'Battery (kWh)', 'Max Charging Power','Session duration (minutes)', 'kWh delivered']]
        
        mean_PD_train = train_sessions_data['Session duration (minutes)'].mean()
        mean_ED_train = train_sessions_data['kWh delivered'].mean()
                    
        X_train = train_sessions_data.drop(columns=['Session duration (minutes)', 'kWh delivered'])
        y_duration_train = train_sessions_data['Session duration (minutes)']
        y_energy_train = train_sessions_data['kWh delivered']

        # Train Random Forest models for energy demand and plug duration
        PD_Known_MLG_model = LinearRegression()
        ED_Known_MLG_model = LinearRegression()

        PD_Known_MLG_model.fit(X_train,(y_duration_train))
        ED_Known_MLG_model.fit(X_train, (y_energy_train))
        
#%%                   TESTING

        test_sessions_data = Data_User.filter(items=[j], axis=0)
        test_sessions_data = test_sessions_data[['Arrival Hour', 'Battery (kWh)', 'Max Charging Power','Session duration (minutes)', 'kWh delivered']]
        
        X_test = test_sessions_data.drop(columns=['Session duration (minutes)', 'kWh delivered'])
        test_PD = test_sessions_data['Session duration (minutes)']
        test_ED = test_sessions_data['kWh delivered']
        test_arrival = test_sessions_data['Arrival Hour']
        
        plug_duration_prediction = (PD_Known_MLG_model.predict(X_test))
        energy_demand_prediction = (ED_Known_MLG_model.predict(X_test))
        
        PD_MLG_Loss = mean_squared_error(test_PD, plug_duration_prediction)
        ED_MLG_Loss = mean_squared_error(test_ED, energy_demand_prediction)
        PD_Naive_Loss = mean_squared_error(test_PD, [mean_PD_train])
        ED_Naive_Loss = mean_squared_error(test_ED, [mean_ED_train])

                                         
        
        temp_d = {'User': User, 'Training': j, 'Actual Arrival Hr': round(test_arrival.iloc[0],2), 'Actual PD': test_PD.iloc[0],'Predicted PD MLG': plug_duration_prediction, 'Predicted PD Naive': mean_PD_train, 
                  'Actual ED': test_ED.iloc[0],'Predicted ED MLG': energy_demand_prediction, 'Predicted ED Naive': mean_ED_train, 
                  'PD MLG Loss':PD_MLG_Loss, 'PD Naive Loss':PD_Naive_Loss, 'ED MLG Loss':ED_MLG_Loss, 'ED Naive Loss':ED_Naive_Loss}
...
for j in range(minimum_training,maximum_training):
    ...
    Mean_MLG_PD_Loss = round(Training_Data["PD MLG Loss"].mean())
    Mean_MLG_ED_Loss = round(Training_Data["ED MLG Loss"].mean(),2)
    Mean_Naive_PD_Loss = round(Training_Data["PD Naive Loss"].mean())
    Mean_Naive_ED_Loss = round(Training_Data["ED Naive Loss"].mean(),2)

    temp_d = {'Training': j, 'Mean PD MLG Loss':Mean_MLG_PD_Loss, 'Mean PD Naive Loss':Mean_Naive_PD_Loss, 'Mean ED MLG Loss':Mean_MLG_ED_Loss, 'Mean ED Naive Loss':Mean_Naive_ED_Loss}
...
print(f'\nMean of mean PD MLG Training Loss: {Error_Table["Mean PD MLG Loss"].mean():.0f}')
print(f'\nMean of mean ED MLG Training Loss: {Error_Table["Mean ED MLG Loss"].mean():.0f}')
...
plt.plot(Error_Table["Training"],Error_Table["Mean PD MLG Loss"],color='green', linestyle = 'solid')
...
plt.plot(Error_Table["Training"],Error_Table["Mean ED MLG Loss"],color='green', linestyle = 'dashed')
...
plt.plot(Error_Table["Training"],np.cumsum(Error_Table["Mean PD MLG Loss"])/(Error_Table["Training"]-9),color='green', linestyle = 'solid')
...
plt.plot(Error_Table["Training"],np.cumsum(Error_Table["Mean ED MLG Loss"])/(Error_Table["Training"]-9),color='green', linestyle = 'dashed')
