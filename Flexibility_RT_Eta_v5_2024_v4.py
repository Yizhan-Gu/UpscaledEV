...
# #2024_v4 
# Edit how the LMP files are called upon due to a nomenclature change from CAISO. Glob module is
# used to generalize this so as long as the file name starts with the date it will be recognized.

# Used ‘10-digit UID’ in place of ‘Car_’ to identify specific users as there were occurrences of
#  multiple unknown users in a single day in 2024 which negated the use of ‘Car_’.

# In 2024, there were occurrences of a car appearing in two different intervals in the sessions data,
#  but after the rounding logic of this algorithm was applied, there were no sessions for this user which 
#  created empty data frames and caused errors. To avoid this, users with two or less intervals were filtered out
#  and a new list, ‘Cars2’ was created to reflect the updated list of cars used in the optimization. 

# There were issues with the format when updating the baseline csv with the current  optimization date(s), 
# as a workaround the original dataframe from the baseline is concated with the updated data frame 
# prior to writing to a csv file; then the entire csv file is overwritten. This is not the most efficient method 
# and there is no reason this bug can’t be fixed for 2024; this workaround was created in the interest of time. 
