# -*- coding: utf-8 -*-
"""
This code is used to optimize the PowerFlex EV charging at UCSD,
along with building, PV and battery in microgrid
With the funding from TotalEnergies UPScaledEV project since 2024 Fall
Created by Yian Chen
Edited by Yizhan Gu
Email: yig031@ucsd.edu
NOTE: For vscode usage only:
Please Type ipython in terminal to run and debug the code, and exit to stop the IPython
The if and while loops lead to indent error in the terminal
https://www.cnblogs.com/bitssea/p/13616586.html
"""
...
#always run both the base case (no demand reduction) and the case1 with service reduction
Cases = ['Base', 'Case1']
...
# 2022 data only has few Hopkins and all Gilman and Athena
filename_Input = dir_Input + '/UCSD_AllSites_Merge_Eta_20210504_20230129_v3.csv' # old data
# filename_Input = 'UCSD-All Sites - Portfolio interval report 01_01_22-01_01_23.csv' # new 2022 PF data
Data = pd.read_csv(filename_Input, low_memory=False)
...
#%%     #################################################################################
        ...
        if DAM == 1:
            ...
            #################################################################################
            # Read the daily dispatch for Baseline days selection and calculation
            #################################################################################
...
#%%     #################################################################################
        # Choose EV data for Day0, Dayb1, and Dayb2
        #################################################################################
        ...
        for i_d in range(3):
            ...
            # if no data on the day (ex: 2022/12/11), take th data from day back 1
...
#%%     #################################################################################
        ...
        # Yizhan: added the BESS and market for testing as main_2.tex
        # BESS variables
        ...
        constraints = [ Cost_Opt >= c_NCD * cp.maximum(cp.atoms.max(EnergyDemand_Step_Opt/dt_h ),M_Th_NCD[2] ) \
                       + c_PD * cp.maximum(cp.atoms.max(Filter * EnergyDemand_Step_Opt[PP_start:PP_end,0 ])/dt_h , M_Th_PD[2]) + c_DR \
...
#%%     #################################################################################
        ...
        Opt_Imp_base, = plt.step(t2, Pw_Opt_Imp_base_stair , linestyle='--',  color='tab:red', linewidth=3)
        Opt_Imp_case1,= plt.step(t2, Pw_Opt_Imp_case1_stair, linestyle='--', color='lightcoral', linewidth=3)
        ...
        NCD_start_base,  = plt.step(t3, M_Pw_NCD_start_Base_stair, linestyle='-', color='tab:orange', linewidth=4)
        NCD_start_case1, = plt.step(t3, M_Pw_NCD_start_Case1_stair, linestyle='', marker='2', color='darkorange', linewidth=4)
        PD_start_base,   = plt.step(t3, M_Pw_PD_start_Base_stair, linestyle='-', color='tab:orange', linewidth=4)
        PD_start_case1,  = plt.step(t3, M_Pw_PD_start_Case1_stair, linestyle='', marker='2', color='darkorange', linewidth=4)
        Event_base,   = plt.plot(TimeSeries_real[0], Event_Base, linestyle='', marker='x', color='lightcoral', linewidth=4)
        Event_case1,  = plt.plot(TimeSeries_real[0], Event_Case1, linestyle='', marker='+', color='lightgreen', linewidth=4)
