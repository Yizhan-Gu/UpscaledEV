...
\item Rolling 24-hour Persistence and Perfect policies are benchmarked against a full-horizon oracle.
...
The study is intentionally scoped. The present results use a June--July 2025 UCSD workplace charging case with daytime sessions and essentially no overnight EV carryover. Wholesale prices and retail tariff schedules are treated as known inputs; the EV-information comparison is between perfect EV session information and a persistence EV forecast. Shrinking-horizon and rolling-horizon MPC are both implemented execution structures rather than competing algorithmic claims. The principal validation evidence, however, focuses on the operationally general Rolling controller under Persistence and Perfect EV information and compares it with a true full-horizon Perfect oracle. Shrinking-horizon results are retained as secondary diagnostics because most workplace flexibility is intraday.
...
\begin{enumerate}[leftmargin=*]
    ...
    \item It separates forecast value from horizon value by comparing Rolling 24-hour Persistence and Perfect policies with a full-horizon Perfect oracle, using executed rather than overwritten planning trajectories.
\end{enumerate}
...
Market participation also requires a tractable representation of aggregate flexibility. Al Taha et al.~\cite{alTaha2025aggregateFlex} construct inner approximations of heterogeneous plug-in-EV flexibility for use in wholesale-market decisions, illustrating that an aggregate offer must remain disaggregatable to feasible device trajectories. This literature establishes that DER aggregations can be represented as market-facing resources, but it does not by itself evaluate a measured campus workplace portfolio behind a retail meter with EV user payments, retail demand charges, BESS feasibility, DA/RT energy positions, AS capacity awards, and executed financial accounting in the same case study.

\subsection{EV smart charging and aggregate flexibility}
The smart-charging literature shows that EV charging can be shifted to reduce grid impacts, lower operating cost, or increase system flexibility \cite{clement2010impact,sortomme2011optimal,gan2013optimal,richardson2013electric,mwasilu2014electric}. In this literature, EV flexibility is created by the difference between plug-in time and required charging time; it is limited by arrival time, departure time, requested energy, and charger power. Aggregate-flexibility studies further show that many individual EV constraints can be approximated or aggregated for market and control decisions \cite{alTaha2025aggregateFlex,nazari2022updated}.
...
Several papers study EV aggregators as market participants. Ancillary-service studies often rely on EV battery discharge or V2G capability and examine whether EV fleets can provide frequency regulation or reserve products while respecting user preferences \cite{clairand2020ancillary,sortomme2012v2gAncillary}. Klemets et al.~\cite{klemets2024charging} coordinate high-power chargers, BESS, and local generation through a hierarchical MPC structure that can provide frequency support. Hermans et al.~\cite{hermans2024implementation} demonstrate workplace-charging MPC on 174 chargers with PV and report substantial peak reduction. These works are close operational precedents, but they do not evaluate the same combination of unidirectional EV load flexibility, CAISO DA/RT energy and four AS capacity products, retail demand charges, EV user revenue, and executed two-settlement accounting.
...
BESS operation has been studied for microgrid scheduling, building demand response, PV self-consumption, demand-charge management, and stacked grid services \cite{parisio2014model,raoufat2018bess,chen2021valueStacking,idomar2026stacked}. Behind the meter, value stacking is difficult because one dispatch action can affect several economic accounts at once. Charging the battery can create future discharge capability or downward reserve capability, but it can also increase retail import and demand-charge exposure. Discharging can reduce a retail peak, but it can also reduce state-of-charge headroom for upward reserve or future energy arbitrage. Recent work in this journal additionally coordinates BESS with PV and flexible loads for frequency-reserve provision \cite{damianakis2026integration}; therefore neither BTM value stacking nor EV--PV--BESS coordination alone is a defensible novelty claim.
...
The existing literature supports the individual ingredients of this work: DER aggregation, EV smart charging, EV aggregator market participation, EV--PV--BESS coordination, BESS value stacking, and MPC execution. The gap addressed here is narrower: a market-timeline-consistent and auditable combination for a measured campus portfolio in which unidirectional workplace EV charging and bidirectional BESS dispatch share one meter and one deliverability envelope, submitted DA quantities remain immutable in RT, RT deviations are settled explicitly, and net value is reconstructed only from executed first-step dispatch.
...
The retail layer includes TOU energy charges, a peak-demand charge, and a non-coincident demand charge. The wholesale layer includes day-ahead (DA) and real-time (RT) energy positions and ancillary-service (AS) capacity products. CAISO identifies four AS products---regulation up, regulation down, spinning reserve, and non-spinning reserve---and requires participating resources to satisfy certification and telemetry requirements; spinning and non-spinning reserve must be deliverable within 10 minutes \cite{caisoAncillary2026}. The model uses these products as an operational abstraction rather than a complete ISO settlement engine. DA quantities are scheduled before operation, RT decisions update dispatch at 15-minute resolution, and AS awards are constrained by EV and BESS deliverability.
...
\begin{enumerate}[leftmargin=*]
    ...
    \item AS awards are constrained by power headroom and BESS duration requirements. The implemented reference case uses one common duration assumption, $T_{\mathrm{DU}}=2\Delta T=0.5$ h, for the four products. This is a modeling assumption to be varied by product in sensitivity analysis, not a claim that all CAISO certification and deployment rules are identical. Expected deployment factors are used for energy accounting; actual regulation mileage, no-pay charges, performance metrics, and full settlement rules are not simulated.
...
Capacity awards must be deliverable for the assumed duration
$T_{\mathrm{DU}}$. The current implementation sets
$T_{\mathrm{DU}}=2\Delta T=0.5$ h for all four products; product-specific
duration and deployment assumptions are evaluated as a sensitivity rather than
embedded as an unqualified CAISO rule:
\begin{align}
\soc_{t+1}^{\BESS}
