...
\begin{highlights}
\item Campus EV charging is evaluated as a DER aggregation resource.
\item Net value is measured after retail, wholesale, and EV-service accounting.
\item Unidirectional EV flexibility is separated from BESS energy shifting.
\item DA/RT energy and AS participation are tested under executed dispatch.
\item Results identify when gross wholesale revenue fails to improve net value.
\end{highlights}

\begin{abstract}
Campus EV charging sites are increasingly large enough to resemble distributed-energy-resource (DER) aggregations, but their economic value depends on more than gross wholesale-market revenue. A behind-the-meter charging operator also pays retail time-of-use and demand charges, collects EV user payments, and must complete charging service without vehicle-to-grid export. This paper evaluates this retail--wholesale coupling for a real campus workplace charging setting with a co-located battery energy storage system (BESS). The contribution is not a new MPC algorithm or a new EV forecasting method; rather, it is an execution-aware application study that quantifies whether unidirectional workplace EV charging and BESS can jointly create net value when day-ahead and real-time wholesale energy positions and ancillary-service capacity products are added to retail bill management. The formulation co-optimizes EV charging, BESS dispatch, retail tariff exposure, EV user payments, wholesale energy revenue, and ancillary-service capacity revenue at 15-minute resolution. A July 2025 UCSD case study compares retail-only operation with retail+wholesale operation under two EV-information settings, perfect EV information and persistence-based EV forecasting, and two execution structures, shrinking-horizon and rolling-horizon MPC. Prices and tariff schedules are treated as known exogenous inputs so that the analysis isolates the operational value and risk created by EV flexibility. The results are interpreted using executed dispatch rather than planning trajectories. This accounting shows when wholesale participation increases gross revenue but also increases retail exposure, and it clarifies why rolling MPC is not expected to dominate in a daytime workplace data set with essentially no overnight charging. The study provides evidence for campus operators considering DER aggregation: value stacking is possible, but its value is determined by the interaction of wholesale products, retail demand charges, BESS feasibility, and EV service constraints.
\end{abstract}
...
\begin{enumerate}[leftmargin=*]
    \item It formulates an execution-aware campus EV--BESS value-stacking problem that combines EV user payments, retail TOU and demand charges, day-ahead and real-time wholesale energy positions, and ancillary-service capacity products in one net-value accounting framework.
    \item It separates unidirectional EV charging flexibility from BESS energy shifting. EVs contribute through controllable demand relative to a baseline, while the BESS provides bidirectional power and state-of-charge headroom.
    \item It evaluates retail-only and retail+wholesale operation on a real workplace charging case, showing that gross wholesale revenue is insufficient as a performance metric when retail demand charges and EV service constraints are present.
    \item It clarifies the operational role of EV information and MPC execution for this application: perfect EV information is an upper benchmark, persistence is an implementable baseline, and rolling MPC is mainly valuable for future cases with overnight or cross-day flexibility rather than for the current daytime workplace data set.
\end{enumerate}
...
\subsection{Smart charging and unidirectional EV flexibility}
Early EV charging studies established that unmanaged charging can increase distribution peaks and that coordinated charging can reduce grid impacts, improve valley filling, or lower charging cost \cite{clement2010impact,sortomme2011optimal,gan2013optimal,richardson2013electric,mwasilu2014electric}. A common theme is that EVs create flexibility through time shifting, but the amount of usable flexibility depends on arrival time, departure time, requested energy, and charger power limits. For workplace charging, this flexibility is typically intraday: many vehicles arrive in the morning and leave in the afternoon, so the EV fleet can shift demand within the workday but provides little storage-like carryover across midnight.

This paper builds on that smart-charging literature but uses EV charging in a different economic role. The EV fleet is not only a load to be scheduled cheaply. It is part of a behind-the-meter portfolio that may also support wholesale energy and ancillary-service positions. Because V2G is not modeled, EVs never inject power. Their upward flexibility is load reduction relative to a charging baseline, and their downward flexibility is additional charging up to the connected-charger limit. The BESS, not the EV fleet, provides bidirectional energy shifting.

\subsection{EV aggregators and wholesale-market participation}
A growing literature studies EV aggregators in electricity markets. Review and perspective papers emphasize that EV aggregations can provide energy, reserve, and ancillary-service products, but that participation is limited by user preferences, market rules, uncertainty, and distribution-system constraints \cite{nazari2022updated,amiri2023participation}. Several studies formulate EV aggregator participation in ancillary-service markets, often with V2G capability or explicit regulation-service modeling \cite{clairand2019participation,delatorre2023optimal}. Other studies examine bidding or pricing strategies for EV charging stations that buy from wholesale markets and sell charging service to EV users \cite{mousavi2022evcsWholesale,bayani2021strategic}. In parallel, DER aggregation research studies how behind-the-meter resources can be represented as market-facing aggregations while accounting for customer welfare, network access, and wholesale bids \cite{chen2023competitiveDER,cui2022gridaware,chen2022dsoDERA}.

These studies motivate the present work but leave an important application gap. Much of the EV-market literature focuses on EV-only aggregators, V2G-capable fleets, strategic bidding, or front-of-meter market participation. The campus problem studied here is different: EV charging and BESS dispatch occur behind a retail meter, the EV fleet is unidirectional, user charging service remains the primary obligation, and the economic outcome depends on both retail tariff charges and wholesale revenue. As a result, the relevant performance metric is net behind-the-meter value rather than wholesale revenue alone.

\subsection{Behind-the-meter value stacking and retail--wholesale coupling}
BESS value stacking has been studied for arbitrage, peak shaving, demand-charge management, and reserve provision \cite{parisio2014model,wang2018predictive,ghosh2022battery,liu2022battery}. Behind the meter, the central complication is that the same dispatch affects multiple accounts. Charging the BESS may create future discharge capability or downward reserve capability, but it can also increase retail import. Discharging can reduce demand charges, but it can reduce the state-of-charge headroom available for upward reserve. When EV charging is added, the BESS and EV fleet share the same retail meter and the same wholesale capability envelope.

The practical issue is therefore not whether each value stream exists separately. It is whether the combined dispatch improves total value after all charges and revenues are counted. For a campus operator, an operationally useful study must report EV user payments, retail TOU charges, peak-demand charges, non-coincident demand charges, wholesale energy revenue, ancillary-service capacity revenue, and EV service completion in one accounting frame. This paper uses that accounting frame to assess whether retail+wholesale operation improves value over retail-only operation.

\subsection{Forecasting, MPC, and implementable execution}
MPC is widely used for microgrid, BESS, building, and EV scheduling because it can update decisions as new states and forecasts arrive \cite{parisio2014model,wang2018predictive,li2021learning}. However, the use of MPC does not by itself establish novelty. For market-facing DER applications, the important modeling choices are what information is available at each decision time, which decisions are committed, and how the financial outcome is computed. EV aggregation studies also show that forecast errors in arrival time, departure time, energy request, and vehicle count can reduce the value of day-ahead commitments \cite{chen2024costoptimal,chen2026realtime}.

The present study therefore uses MPC as an execution mechanism, not as the main contribution. Perfect EV information is included as a benchmark for information value; persistence EV forecasting is included as a simple implementable baseline. The comparison between shrinking and rolling horizons is interpreted through the structure of the UCSD workplace data. Because the tested month has essentially no overnight charging, a rolling horizon is not expected to outperform merely because it updates more often. This interpretation prevents the results from being framed as a generic MPC ranking and keeps the focus on the campus DER aggregation application.

\subsection{Research gap and novelty of this paper}
The literature supports the individual components of the problem: EV smart charging, EV aggregator market participation, BESS value stacking, and MPC under uncertainty. The gap is their execution-aware combination for a unidirectional workplace EV fleet and BESS behind a retail meter. Specifically, few studies quantify how wholesale energy and ancillary-service participation changes net value after retail demand charges, EV user payments, BESS feasibility, EV service, and day-ahead/real-time execution are all accounted for.

The novelty of this paper is therefore the applied retail--wholesale evaluation. The work demonstrates how a campus charging operator can translate workplace EV flexibility and BESS headroom into market-facing quantities, and it tests whether doing so improves net value relative to retail-only operation. The results, discussion, and conclusions are organized around this novelty claim: gross wholesale revenue is useful only if it survives the retail meter and EV-service accounting.
