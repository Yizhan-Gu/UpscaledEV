...
$p_t^{\DA,\mathrm{com}}$, $p_t^{\WM}$, $p_t^{\RT,\mathrm{dev}}$ & Active DA energy commitment, physical net wholesale position, and RT-settled deviation.\\
...
\begin{align}
r_t^{\WM}=&\DeltaT\left(C_t^{\mathrm{LMP},\DA}p_t^{\DA,\mathrm{com}}+C_t^{\mathrm{LMP},\RT}\left(p_t^{\WM}-p_t^{\DA,\mathrm{com}}\right)\right)\nonumber\\
...
Wholesale revenue includes the fixed DA energy commitment, the RT deviation settlement, and AS capacity revenue:
\begin{align}
R^{\WM}=&\sum_{t\in\calT}\DeltaT\left(C_t^{\mathrm{LMP},\DA}p_t^{\DA,\mathrm{com}}+C_t^{\mathrm{LMP},\RT}\left(p_t^{\WM}-p_t^{\DA,\mathrm{com}}\right)\right)\nonumber\\
...
The DA and RT problems share the same physical EV/BESS model, but the RT execution layer distinguishes a financial DA commitment from the physically executed market position. Let $m_t^{\DA}\in\{0,1\}$ identify an interval whose DA commitment is still active and let $\bar p_t^{\DA}$ be the corresponding DA schedule. The commitment carried into RT is
\begin{align}
p_t^{\DA,\mathrm{com}}=m_t^{\DA}\bar p_t^{\DA}.
\label{eq:da_commitment}
\end{align}
The RT optimization chooses a feasible net wholesale position $p_t^{\WM}$ and settles only its deviation from the DA commitment at the RT price:
\begin{align}
p_t^{\RT,\mathrm{dev}}=p_t^{\WM}-p_t^{\DA,\mathrm{com}}.
\label{eq:rt_deviation}
\end{align}
Thus, DA energy remains in the financial account even when newly observed EV availability or BESS SOC makes the original physical schedule undesirable. The representation is intentionally an operational settlement abstraction, not a complete ISO settlement or qualification model. It avoids treating a submitted DA schedule as an unconstrained RT variable and avoids using a numerical big-M penalty as a proxy for settlement.
...
\begin{table*}[!t]
\centering
\caption{Executed June--July 2025 net-value results (US$). ``Both'' enables wholesale DA/RT energy and AS products; all values are computed from the saved 15-minute dispatch.}
...
\midrule
Shrinking & Perfect & 13,448.88 & 21,541.89 & 8,093.01\\
Shrinking & Persistence & 10,434.88 & 20,245.09 & 9,810.21\\
Rolling & Perfect & 12,606.19 & 19,411.04 & 6,804.85\\
Rolling & Persistence & 11,038.22 & 18,710.34 & 7,672.12\\
\bottomrule
\end{tabular}
\end{table*}
...
Table~\ref{tab:monthly_results} and Fig.~\ref{fig:value_stack} show a positive aggregate value-stack increment for all four controller--forecast combinations. The wholesale-enabled net value is highest for shrinking MPC with perfect EV information (\$21,541.89), followed by shrinking MPC with persistence (\$20,245.09), rolling MPC with perfect EV information (\$19,411.04), and rolling MPC with persistence (\$18,710.34). The key interpretation is not that gross market revenue alone is positive, but that it exceeds the induced change in retail cost after executed accounting. In this daytime workplace case, the shrinking framework is better aligned with largely intraday flexibility; the result is not intended as a general ranking of rolling MPC.
...
\end{align}
Positive and negative components are represented with auxiliary variables in the implementation rather than nonlinear max functions. Under RT settlement, $p_t^{\WM}$ is the physical net market position; the DA energy commitment is retained separately in the financial settlement expression below.
...
\begin{align}
p_t^{\DA,\mathrm{com}}&=m_t^{\DA}\bar p_t^{\DA},\label{eq:app_da_commitment}\\
p_t^{\RT,\mathrm{dev}}&=p_t^{\WM}-p_t^{\DA,\mathrm{com}}.\label{eq:app_rt_deviation}
\end{align}
The DA term is fixed by the submitted schedule; $p_t^{\WM}$ is the optimized physical position and $p_t^{\RT,\mathrm{dev}}$ is the quantity settled at the RT price. This avoids the former big-M absolute-deviation penalty and does not require an infeasibility-prone equality between the physical RT dispatch and a stale DA trajectory. DA AS capacity can be retained up to the active DA reference; any increment is represented by the RT AS variables.

\subsection{Revenue decomposition}
Wholesale revenue is decomposed as
\begin{align}
R^{\WM,\energy}&=\sum_t\DeltaT\left(C_t^{\mathrm{LMP},\DA}p_t^{\DA,\mathrm{com}}+C_t^{\mathrm{LMP},\RT}p_t^{\RT,\mathrm{dev}}\right)+R^{\mathrm{deploy}},\label{eq:app_energy_rev}\\
