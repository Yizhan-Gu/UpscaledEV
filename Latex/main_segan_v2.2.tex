...
The model is solved repeatedly under either shrinking-horizon or rolling-horizon MPC. Let $\calH_t$ denote the active prediction horizon at control time $t$. For shrinking MPC, $\calH_t$ contains the remaining intervals in the operating day. For rolling MPC, $\calH_t$ is a moving look-ahead window that shifts forward after each execution step. The optimization problem has the common form
\begin{equation}
\label{eq:mpc_master}
\max_{\mathbf{u}_{t:\calH_t}}
\quad
J_t(\mathbf{u}_{t:\calH_t};\widehat{\Xi}_{t:\calH_t},x_t)
-
\Pi_t^{\RT},
\end{equation}
where $\mathbf{u}_{t:\calH_t}$ collects EV charging, BESS dispatch, wholesale-market quantities, AS capacities, and retail-meter variables over the horizon; $\widehat{\Xi}_{t:\calH_t}$ is the EV information set generated by either the perfect-information or persistence module; $x_t$ is the realized physical and financial state at the start of the solve; and $\Pi_t^{\RT}$ denotes RT reference-tracking penalties used when previously submitted DA quantities should be followed but not enforced as hard equalities.
