...
\begin{align}
    &\min_{\substack{P^{\rm EV}_{t,\text{opt}}, P^{\text{D}}_{t}, P^{\text{C}}_{t}, P^{\text{RU}}_t, P^{\text{RD}}_t, P^{\text{SP}}_t, P^{{\text{NSP}}}_t}} Cost_t - Revenue_t \label{eq:obj}
\end{align}
...
\noindent \textbf{EV Constraints:}
\begin{align}
    (\mathbf{1}_t^\top P^{\rm EV}_{t,\text{opt}})_i &= \frac{\eta_i (e_{di} - e_{ai})}{\Delta{t}} - (\mathbf{1}_{t-1}^\top P^{\rm EV}_{\text{imp}})_i \label{eq:EV_dynamic} \\
%    
    0 &\leq P^{\rm EV}_{t,\text{opt}} \leq \hat{p}_t \label{eq:EV_physical}
\end{align}
%
\textbf{BESS Constraints:}
\begin{align}
    &P^{\text{BESS}}_t = \sqrt{\gamma}(P^{\text{C}}_{t} +  P^{{\text{RD}}}_t) 
    - \frac{1}{\sqrt{\gamma}}(P^{\text{D}}_{t} +  P^{{\text{RU}}}_t +  P^{{\text{SP}}}_t + P^{{\text{NSP}}}_t) \label{eq:BESS_load} \\
%    
    %&P^{\text{BESS}}_{\text{bid}, h} = (P^{\text{BESS}}_t)_h \label{eq:BESS_bid} \\
    &-P^{\text{BESS}}_{\text{max}} \leq P^{\text{BESS}}_t \leq P^{\text{BESS}}_{\text{max}} \label{eq:BESS_physical} \\
%    
    &0 \leq \sqrt{\gamma}(P^{\text{C}}_t)\leq P^{\text{BESS}}_{\text{max}}\\
%    
    &0 \leq \sqrt{\gamma}(P^{\text{RD}}_t)\leq P^{\text{BESS}}_{\text{max}}\\
%
    &0 \leq \frac{1}{\sqrt{\gamma}}(P^{\text{D}}_t)\leq P^{\text{BESS}}_{\text{max}}\\
%
    &0 \leq \frac{1}{\sqrt{\gamma}}(P^{\text{RU}}_t)\leq P^{\text{BESS}}_{\text{max}}\\%
%    
    &0 \leq \frac{1}{\sqrt{\gamma}}(P^{\text{SP}}_t)\leq P^{\text{BESS}}_{\text{max}}\\%
%  
    &0 \leq \frac{1}{\sqrt{\gamma}}(P^{\text{NSP}}_t)\leq P^{\text{BESS}}_{\text{max}}\\       
%
    &P^\text{D}_t \cdot P^{\text{C}}_t =0\\
%    
    &P^{{\text{RU}}}_t \cdot P^{{\text{RD}}}_t =0\\
 %   
    &SOC^{\text{BESS}}_{\text{min}}  \leq SOC^{\text{BESS}}_t 
    \leq SOC^{\text{BESS}}_{\text{max}} \label{eq:SOC_physical} \\ 
%
    &SOC^{\text{BESS}}_t = SOC^{\text{BESS}}_{t-1} + \Delta{t} \frac{P^{\text{BESS}}_t}{C^{BESS}} \label{eq:SOC_dynamic} \\
%    
\end{align}
...
\noindent \textbf{Both DR and AS Market:}


\begin{align}
%
    L_{\text{imp},t}^{\text{EV}} &= (P^{\rm EV}_{t,\text{opt}} \pmb{\eta}_{N_{\text{EV}}} \label{eq:load_ev_imp_eta})_{1,:} \\
    L^{\text{net}}_{\text{act},t} &= L_{\text{imp},t}^{\text{EV}} + L_{t}^{\text{building}} + P^{\text{BESS}}_t - P^{\text{PV}}_t \label{eq:actual_net_load} \\
%
    Cost_t &= c_{\text{NCD}} \cdot \max \left(L^{\text{net}}_{\text{act},t}, \bar{L}^{\text{net}}_{\text{NCD}} \right) \label{eq:ncd} \\\nonumber
    &+ c_{\text{PD}} \cdot \max \left(z^{\text{PD}}_t \cdot L^{\text{net}}_{\text{act},t}, \bar{L}^{\text{net}}_{\text{PD}} \right) \\\nonumber
    &+ c_{\text{TOU}} \cdot \Delta t \sum_t L^{\text{net}}_{\text{act},t} \label{eq:tou} \\
%
    Revenue_t &= c_{\text{EV}} \cdot \Delta t \sum_t L^{\text{EV}}_{\text{imp},t} \nonumber \\
    &+ c_{\text{LMP}}^{\text{DA}} \cdot \Delta t \sum_t z_{\text{event},h}^{\text{DR}} \cdot P_{\text{act},t}^{\text{DR}} \\ \nonumber
    &+ c_{\text{AS}}^{\text{DA}} \cdot \Delta t \sum_t P^{DA}_{\rm AS}+c_{\text{AS}}^{\text{RT}} \cdot \Delta t \sum_t P^{RT}_{\rm AS}\\ \nonumber
    + &\alpha_t \cdot c_{\text{LMP}} \cdot \Delta t \sum_t (P^{RT}_{\rm AS}+P^{DA}_{\rm AS})\\ \nonumber
    +&c^{\rm DA}_{\text{EM}} \cdot \Delta t \sum_t (P^{RT}_{\rm EM}+P^{DA}_{\rm EM})\\
%    
\end{align}
